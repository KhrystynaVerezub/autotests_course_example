...
def everything_for_your_cat(cats_data):  # Объявляем функцию, которая принимает на вход список cats_data
    cats_dict = {}  # Создаем пустой словарь, который мы будем заполнять данными о котах

    """Запускаем цикл for, который будет перебирать все элементы списка cats_data,
    где каждый элемент содержит данные об одном коте"""
    for j in range(len(cats_data)):
        record = cats_data[j]  # Получаем текущую запись из списка cats_data
        key = record[2] + " " + record[3]  # Определяем ключ (имя владельца) для текущего кота, объединив имя и фамилию владельца
        value = (record[0], record[1])   # Создаем кортеж значения (имя кота, порода), который мы будем добавлять в словарь

        """Получаем текущий список котов для данного владельца из словаря.
        Если такого списка нет, то создаем пустой список"""
        new_record = cats_dict.get(key)
        if new_record is None:
            new_record = []

        new_record.append(value)  # Добавляем новую запись в конец списка для текущего владельца
        cats_dict[key] = new_record  # Обновляем значение для данного ключа (имени владельца) в словаре cats_dict

    our_str = ""  # Создаем пустую строку, которая будет содержать итоговый результат
    owners = cats_dict.keys()  # Получаем список всех владельцев (ключей словаря) из словаря cats_dict

    """Запускаем цикл for, который перебирает всех владельцев из списка owners"""
    for owner in owners:
        our_str = our_str + owner + ": "  # Добавляем имя владельца в нашу выходную строку, за которым следует двоеточие
        cats = cats_dict.get(owner)  # Получаем список котов для текущего владельца

        """Запускаем цикл for, который будет перебирать всех котов данного владельца из списка cats"""
        for k in range(len(cats)):
            our_str = our_str + cats[k][0] + ", " + str(cats[k][1])  # Добавляем в выходную строку имя кота и его породу

            """Если это не последний кот у данного владельца, то добавляем точку с запятой и пробел,
            а если это последний кот, то ставим перевод строки"""
            if k < len(cats) - 1:
                our_str = our_str + "; "
            else:
                our_str = our_str + "\n"
