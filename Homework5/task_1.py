...
def letter_stat(our_str):
    ...
    for i in our_str:  # циклом перебираем все символы строки our_str
        if i.isalpha():  # при каждом проходе проверяем, является ли текущий символ буквой
            letters_dict[i] = letters_dict.get(i, 0) + 1  # если буква - то добавляем в словарь +1 для этой буквы
    return letters_dict
