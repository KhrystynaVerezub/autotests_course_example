...
def repeats(our_str):
    new_list = [i for i in our_str]  # используем генератор списков чтоб создать список

    """ Используем метод join(), чтобы объединить все элементы из списка new_list в одну строку
    + добавляем "_n" после каждого элемента + "_n" в конце строки, чтобы закончить ее на постфиксе _n"""
    new_str = '_n'.join(new_list) + '_n'
    return new_str
