...
""" Берем цикл while, который будет перемножать цифры числа num до тех пор, пока не получится одна цифра.
Для каждой повторения будем увеличивать счетчик на 1"""
...
def multiplication_chain(num):
    ...
    while len(str(num)) > 1:  # пока число содержит более одной цифры. len - чтобы определять количество цифр в числе
        ...
        for digit in str(num):  # перемножаем все цифры числа между собой
            multy *= int(digit)
        num = multy  # записываем новое число
